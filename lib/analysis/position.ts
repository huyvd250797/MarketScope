...
export function analyzePositionExit(
  ...
  return {
    ...
    warnings,
    guardrails: [
      'V0.4.0 phân tích vị thế LONG đã vào; không tự đặt lệnh, không dùng leverage recommendation và không tạo lệnh SHORT.',
      'Ngắn/trung/dài hạn là khung lập kế hoạch theo timeframe, không phải ETA chắc chắn để chạm target.',
      'Không suy diễn win rate hoặc xác suất thắng từ Signal Score/Position status; phần này chỉ xuất hiện sau backtest/calibration ở V0.5.0.',
      'V0.4.0 chưa có portfolio sizing nên không tự đề xuất tỷ trọng % cần bán ở từng mốc.',
    ],
