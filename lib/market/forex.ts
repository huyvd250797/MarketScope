...
async function fetchChart(providerSymbol: string, interval: string, range: string): Promise<YahooResult> {
  ...
  for (const host of ['query1.finance.yahoo.com', 'query2.finance.yahoo.com']) {
    try {
      ...
      const response = await fetch(url, { signal: AbortSignal.timeout(8000), next: { revalidate: 30 }, headers: { Accept: 'application/json', 'User-Agent': 'Mozilla/5.0 (compatible; MarketScope/0.12.0)' } });
