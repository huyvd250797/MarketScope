...
--   The current app version does NOT switch normal app writes to these new tables yet. V15.0.69 keeps manual migration RPCs and repairs existing legacy tables safely.
--
...
-- ---------- Comments ----------
comment on table public.change_logs is 'Append-only relational change feed for future realtime/queue sync. V15.0.69 keeps legacy JSON as backup and adds manual JSON-to-relational migration RPCs; normal app writes remain on legacy JSON until RelationalWriteQueue.';
...
declare
  ...
  v_batch_id uuid := public.myb_stable_uuid('migration:v15.0.69:' || coalesce(nullif(p_sync_id,''),'main'));
  v_op_id uuid := public.myb_stable_uuid('migration-op:v15.0.69:' || coalesce(nullif(p_sync_id,''),'main'));
  v_row record;
  v_sub record;
  v_item jsonb;
  v_member_id uuid;
...
begin
  ...
  values(v_device_id, v_family_id, 'RelationalReadMode', 'migration', 'supabase-sql', coalesce(v_data->>'_appVersion', v_data->>'appVersion', '15.0.69'), now(), now(), now())
  ...
  values(v_batch_id, v_family_id, v_sync_id, coalesce(v_data->>'_appVersion', v_data->>'appVersion', 'legacy-json'), 'completed', jsonb_build_object('version','15.0.69','imported',v_counts,'source_counts',public.myb_migration_source_counts(v_data)), now(), now(), now(), now(), null)
  ...
  values(v_family_id, 'migration_batches', v_batch_id, 'json_to_relational_migration', v_op_id, v_device_id, jsonb_build_object('version','15.0.69','sync_id',v_sync_id,'imported',v_counts));
  c_logs := c_logs + 1;
...
comment on function public.myb_preview_json_migration(text) is 'V15.0.69 preview legacy JSON counts before importing to relational tables.';
comment on function public.myb_migrate_json_to_relational(text, boolean) is 'V15.0.69 manual, idempotent migration from meyeube_sync.data JSONB to relational tables. Does not switch app read/write mode.';
comment on function public.myb_relational_migration_status(text) is 'V15.0.69 relational migration status and target table counts.';
...
-- =============================================================
-- Mẹ Yêu Bé V15.0.69 · RelationalProductionPush
-- Purpose:
...
create table if not exists public.relational_primary_state (
  ...
  mode_version text not null default '15.0.69',
...
begin
  ...
  values(v_family_id, v_sync_id, case when jsonb_array_length(v_blockers)=0 then 'primary_ready' else 'preflight_blocked' end, '15.0.69', now(), v_legacy_updated_at, v_read, v_queue, v_counts, v_blockers, case when jsonb_array_length(v_blockers)=0 then now() else null end, now(), now(), null)
  on conflict (family_id) do update set
...
begin
  ...
  values(v_device_id, v_family_id, coalesce(nullif(p_device_key,''),'Thiết bị chốt relational'), 'pwa', 'web', '15.0.69', now(), now(), now(), null)
  ...
  insert into public.change_logs(family_id, table_name, row_id, operation, op_id, device_id, payload)
  values(v_family_id, 'relational_primary_state', v_family_id, 'promote_primary', v_op_id, v_device_id, jsonb_build_object('version','15.0.69','sync_id',v_sync_id,'note',p_note,'preflight',v_preflight));
...
comment on table public.relational_primary_state is 'V15.0.69 server-side marker for promoting relational tables as official primary data source while keeping legacy JSON backup.';
comment on function public.myb_relational_primary_preflight(text) is 'V15.0.69 final gate before official relational production push: Doctor passed, Delta=0, WriteQueue clean.';
comment on function public.myb_relational_promote_primary(text, text, text) is 'V15.0.69 promotes relational DB as primary/official source. Does not delete meyeube_sync legacy backup.';
